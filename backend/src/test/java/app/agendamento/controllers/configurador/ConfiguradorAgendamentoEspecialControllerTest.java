...
@QuarkusTest
class ConfiguradorAgendamentoEspecialControllerTest extends AuthContextManager {

    ConfiguradorAgendamentoEspecialDTO newEntity;

    @BeforeEach
    void setUp() {
        getAuthTokens(defaultContext);
        RestAssured.basePath = "/configuradorAgendamentoEspecial/";
    }
    ...
    void createEntity(AuthContextTest context, ConfiguradorAgendamentoEspecialDTO entity) {
        ...
        newEntity = jsonPath.getObject("data", ConfiguradorAgendamentoEspecialDTO.class);

        deletedIds.add(newEntity.getId());
    }

    void updateEntity(AuthContextTest context, ConfiguradorAgendamentoEspecialDTO entity) {
    ...
    void createNewEntity() {
        newEntity = new ConfiguradorAgendamentoEspecialDTO();
        newEntity.setNome("Semana de fisioterapia");
        newEntity.setProfissionalConfigurador(new UsuarioDTO(5L));
        newEntity.setDataInicio(Contexto.proximaDataByIntervalo(1, DayOfWeek.MONDAY));
        newEntity.setDataFim(Contexto.proximaDataByIntervalo(1, DayOfWeek.FRIDAY));
        newEntity.setOrganizacaoConfigurador(new OrganizacaoDTO(1L));
        newEntity.addTiposAgendamentos(new TipoAgendamentoDTO(1L));
    }

    void updateNewEntity() {
        newEntity.setDataFim(Contexto.proximaDataByIntervalo(1, DayOfWeek.THURSDAY));
    }
}
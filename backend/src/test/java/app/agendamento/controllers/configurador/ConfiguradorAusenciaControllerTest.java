...
@QuarkusTest
class ConfiguradorAusenciaControllerTest extends AuthContextManager {

    ConfiguradorAusenciaDTO newEntity;

    @BeforeEach
    void setUp() {
        getAuthTokens(defaultContext);
        RestAssured.basePath = "/configuradorAusencia/";
    }
    ...
    void createEntity(AuthContextTest context, ConfiguradorAusenciaDTO entity) {
        ...
        newEntity = jsonPath.getObject("data", ConfiguradorAusenciaDTO.class);

        deletedIds.add(newEntity.getId());
    }

    void updateEntity(AuthContextTest context, ConfiguradorAusenciaDTO entity) {
    ...
    void createNewEntity() {
        newEntity = new ConfiguradorAusenciaDTO();
        newEntity.addProfissionaisAusentes(new UsuarioDTO(5L));
        newEntity.setDataInicioAusencia(Contexto.proximaDataByIntervalo(1, DayOfWeek.MONDAY));
        newEntity.setDataFimAusencia(Contexto.proximaDataByIntervalo(1, DayOfWeek.FRIDAY));
        newEntity.setNomeAusencia("Férias de 1 semana");
        newEntity.setObservacao("O profissional merece umas férias ;)");
    }

    void updateNewEntity() {
        newEntity.setDataFimAusencia(Contexto.proximaDataByIntervalo(2, DayOfWeek.FRIDAY));
        newEntity.setNomeAusencia("Férias de 2 semanas");
        newEntity.setObservacao("O profissional merece mais férias ;)");
    }
}
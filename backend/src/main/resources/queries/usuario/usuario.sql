...
WHERE U.ID = :PROFISSIONALID
  ...
  AND NOT EXISTS (SELECT 1
                  FROM CONFIGURADORAUSENCIA C4
                           LEFT JOIN CONFIGURADORAUSENCIAUSUARIO C5
                                     ON C5.CONFIGURADORAUSENCIAID = C4.ID
                  WHERE C5.USUARIOID IN (:USUARIOID)

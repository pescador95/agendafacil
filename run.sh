...
run() {
    ...
    if [ "$1" == "dev" ]; then
        ./kubernetes/manifests/up-dev.sh <<< "redis,backend,frontend,telegram,grafana,prometheus,loki,otel,tempo"
    elif [ "$1" == "prod" ]; then
        ./kubernetes/manifests/up.sh <<< "redis,backend,frontend,telegram,whatsapp,grafana,prometheus,loki,otel,tempo"
    else
